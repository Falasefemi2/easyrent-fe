...
export class TokenStore extends Context.Service<
  TokenStore,
  {
    ...
    readonly setTokens: (tokens: { accessToken: string; refreshToken: string }) => Effect.Effect<void>
...
export class ApiService extends Context.Service<
  ApiService,
  {
    ...
    readonly signIn: (params: { email: string; password: string }) => Effect.Effect<AuthTokens, ApiError>
    ...
    readonly getListingById: (id: string) => Effect.Effect<ListingWithMedia, ApiError>
    ...
    readonly getMyListings: (params?: { page?: number; limit?: number }) => Effect.Effect<PaginatedListings, ApiError>
    ...
    readonly removeFavorite: (listingId: string) => Effect.Effect<void, ApiError>

    readonly getMyFavorites: (params?: { page?: number; limit?: number }) => Effect.Effect<PaginatedFavorites, ApiError>

    readonly checkFavorite: (listingId: string) => Effect.Effect<boolean, ApiError>

    readonly uploadAvatar: (file: File) => Effect.Effect<{ avatarUrl: string }, ApiError>
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const makeAuthRequest = <A, E>(
        ...
        Effect.gen(function* () {
          ...
          const withToken = (t: string | null) =>
            baseClient.pipe(
              HttpClient.mapRequest(t ? HttpClientRequest.setHeader("Authorization", `Bearer ${t}`) : (req) => req),
            )
      ...
      const signIn = Effect.fn("ApiService.signIn")(
        (params: { email: string; password: string }): Effect.Effect<AuthTokens, ApiError> =>
          Effect.gen(function* () {
            const response = yield* HttpClientRequest.post("/auth/sign-in").pipe(
      ...
      const signUp = Effect.fn("ApiService.signUp")(
        (params: {
          ...
          Effect.gen(function* () {
            const response = yield* HttpClientRequest.post("/auth/sign-up").pipe(
              HttpClientRequest.bodyJsonUnsafe(params),
              baseClient.execute,
              Effect.flatMap(HttpClientResponse.schemaBodyJson(AuthTokens)),
              Effect.mapError(mapError),
      ...
      const getListings = Effect.fn("ApiService.getListings")(
        (params?: {
          ...
          if (params?.furnished !== undefined) urlParams.furnished = String(params.furnished)
          if (params?.rooms !== undefined) urlParams.rooms = String(params.rooms)
          if (params?.minRooms !== undefined) urlParams.minRooms = String(params.minRooms)
      ...
      const deleteListing = Effect.fn("ApiService.deleteListing")(
        (id: string): Effect.Effect<void, ApiError> =>
          makeAuthRequest((client) =>
            HttpClientRequest.delete(`/listings/${id}`).pipe(client.execute, Effect.asVoid, Effect.mapError(mapError)),
          ),
      )

      const getMyListings = Effect.fn("ApiService.getMyListings")(
        (params?: { page?: number; limit?: number }): Effect.Effect<PaginatedListings, ApiError> =>
          makeAuthRequest((client) =>
            client
              .get("/listings/my", {
                urlParams: {
        ...
        > =>
          makeAuthRequest((client) =>
            Effect.gen(function* () {
              ...
              return yield* HttpClientRequest.post(`/listings/${listingId}/media`).pipe(
      ...
      const getMyFavorites = Effect.fn("ApiService.getMyFavorites")(
        (params?: { page?: number; limit?: number }): Effect.Effect<PaginatedFavorites, ApiError> =>
          makeAuthRequest((client) =>
            client
              .get("/favorites", {
                urlParams: {
        ...
        (file: File): Effect.Effect<{ avatarUrl: string }, ApiError> =>
          makeAuthRequest((client) =>
            Effect.gen(function* () {
              ...
              const response = yield* HttpClientRequest.post("/users/avatar").pipe(
                HttpClientRequest.bodyFormData(formData),
                client.execute,
              )
      ...
      const updateListingStatus = Effect.fn("ApiService.updateListingStatus")(
        (id: string, status: "avaiable" | "rented" | "inative"): Effect.Effect<Listing, ApiError> =>
          makeAuthRequest((client) =>
