    ...
    <Link to="/listings/$id" params={{ id: listing.id }}>
      <div className="group cursor-pointer">
        <div className="relative aspect-4/3 rounded-xl overflow-hidden bg-gray-100 mb-3">
          ...
          >
            <Heart
              className={`w-4 h-4 transition-colors ${favorited ? "fill-[#E8442A] text-[#E8442A]" : "text-gray-600"}`}
            />
          </button>

          {listing.furnished && (
            <div className="absolute top-3 left-3">
              <Badge className="bg-white text-gray-700 text-xs font-normal shadow-sm">Furnished</Badge>
            </div>
          )}
        </div>
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-500 line-clamp-1">{listing.address}</p>

          <div className="flex items-center gap-2">
            <p className="text-sm font-semibold text-gray-900">₦{priceFormatted}</p>
          ...
          <div className="flex gap-2">
            ...
            {listing.status === "rented" && (
              <span className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">Rented</span>
            )}
            {listing.status === "inative" && (
              <span className="text-xs text-gray-500 bg-gray-50 px-2 py-0.5 rounded-full">Inactive</span>
            )}
          </div>
        </div>
      </div>
